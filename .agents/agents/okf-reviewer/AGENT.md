...
11. Explicit Alpha, Beta, Preview, Stable, and Deprecated labels are preserved. Without an explicit label, served `v1alpha*` APIs are Alpha and served `v1beta*` APIs are Beta; neither may be recorded as Stable. Other APIs default to Stable only when no selected current source or served API version indicates a non-stable state. A served-version state applies only to the API or capability that owns it; it is not transferred between a Function package and its input API. Unlabelled non-API software, packages, helpers, and implementation behavior are `Not stated by selected sources`; a stable semantic-version tag is source selection, not maturity. Patterns inherit the least stable required API or explicitly labelled capability. `v1` alone is not proof of Stable, and design status never defines feature state.
